...
def main() -> None:
    ...
    with st.sidebar:
        ...
        source_mode = st.radio("Source type", ["YouTube URL", "Local video"], index=0)
        ...
        if source_mode == "Local video":
            uploaded_file = st.file_uploader("Or upload a video", type=["mp4", "mov", "avi", "mkv", "m4v"])

        weights = st.text_input("Weights path", value=str(DEFAULT_WEIGHTS))
        out_root = st.text_input("Output folder", value=str(OUTPUT_ROOT))
        ...
        with st.expander("Advanced"):
            ...
            device = st.text_input("Device", value="", help="Leave empty for auto, or use cpu / mps / 0.")
    ...
    if run_clicked:
        ...
        try:
            ...
            def on_progress(done: int, total: int, detections: int) -> None:
                ...
                progress_placeholder.write(f"Processed {done}/{total} frames. Detections: {detections}")
