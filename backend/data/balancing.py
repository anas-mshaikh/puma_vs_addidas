...
def count_classes(label_path: Path) -> Counter:
    counts = Counter()
...
def print_progress(prefix: str, current: int, total: int) -> None:
    ...
    sys.stderr.write(
        f"\r{prefix} [{bar}] {current}/{total} ({percent:5.1f}%)".ljust(80)
    )
    sys.stderr.flush()
...
def main() -> None:
    ...
    for split in ["train", "valid", "test"]:
        ...
        selected_puma = puma_images if KEEP_ALL_PUMA_IMAGES else []
        selected = selected_puma + mixed_images + selected_adidas
        random.shuffle(selected)

        total_selected = len(selected)
        for idx, (image_path, label_path, adidas_boxes, puma_boxes) in enumerate(
            selected, start=1
        ):
